  ...
  )
{
  ...
  ZeroMem (&Dhcp4Mode, sizeof (Dhcp4Mode));
  Status = Dhcp4->GetModeData (Dhcp4, &Dhcp4Mode);
  if (EFI_ERROR (Status) ||
      IsZeroIpv4Address (&Dhcp4Mode.ClientAddress) ||
      IsZeroIpv4Address (&Dhcp4Mode.ServerAddress))
  {
    return EFI_NOT_FOUND;
  }

  CopyMem (DhcpServerAddress, &Dhcp4Mode.ServerAddress, sizeof (*DhcpServerAddress));
  return EFI_SUCCESS;
}
  ...
  if (!EFI_ERROR (Status) && (LoadedImage->DeviceHandle != NULL)) {
        ...
        PxeBaseCode->Mode->DhcpAckReceived)
    {
      *ControllerHandle = LoadedImage->DeviceHandle;
      ...
      Status = TryGetDhcp4ServerAddress (ImageHandle, *ControllerHandle, DhcpServerAddress);
      if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  if (!EFI_ERROR (Status) && (InterfaceInfo != NULL)) {
        ...
        !IsZeroIpv4Address (&InterfaceInfo->SubnetMask))
    {
      ...
      TryGetDhcp4ServerAddress (ImageHandle, ControllerHandle, DhcpServerAddress);
